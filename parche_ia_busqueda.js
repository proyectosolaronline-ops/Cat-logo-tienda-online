// PARCHE IA v6.1 — fix: tienda sin marcador ahora se agrega al mapa antes de abrir popup
...
async function runAI(q){
  ...
  // FIX: usar ALL_STORES completo para que la IA vea todas las tiendas, no solo las cercanas
  var stores = window.ALL_STORES || window.nearbyStores || [];

  var ctx = stores.slice(0,80).map(function(s){
    var cfg=s.config||{};
...
/* ── CLICK FILA → volar al pin + agregar marcador si no existe ── */
window._aiGo = function(lat, lng, id){
  if(!window.map) return;
  var la = parseFloat(lat), ln = parseFloat(lng);
  if(isNaN(la) || isNaN(ln)) return;

  // FIX PRINCIPAL: si la tienda no tiene marcador en el mapa, lo creamos antes de abrir popup
  if(id && window.ALL_STORES){
    var store = window.ALL_STORES.find(function(s){ return s.storeId === id; });
    if(store){
      var yaEsta = window.nearbyStores && window.nearbyStores.find(function(s){ return s.storeId === id; });
      if(!yaEsta){
        if(!window.nearbyStores) window.nearbyStores = [];
        window.nearbyStores.push(store);
        if(typeof window.addMarkersToMap === 'function'){
          window.addMarkersToMap([store]); // agrega solo ese marcador
        }
      }
    }
  }

  // cerrar panel de búsqueda y limpiar
  hide();
  resetPins();
  ...
  // abrir popup después de que el mapa vuele
